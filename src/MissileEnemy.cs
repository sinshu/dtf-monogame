...
namespace MiswGame2008
{
    public class MissileEnemy : Enemy
    {
        ...
        public MissileEnemy(Game game, double x, double y)
            : base(game, x, y)
        {
            angle = 90;
            vx = vy = 0;
            ...
            hitPoints = 2;
            damaged = false;
            animation = game.Random.Next(0, 10);
        }
            ...
            if (waitCount == 0)
            {
                ...
                else
                {
                    ...
                    else if (!Game.Player.IsDead)
                    {
                        if (Y < Game.Player.Y)
                        {
                            Bullet bullet = new Missile(Game, X + 32 * Utility.Cos(angle), Y - 32 * Utility.Sin(angle), angle);
        ...
        public override void Draw(IGraphics graphics)
        {
            ...
            graphics.DrawObject(Image.MissileEnemy, drawX, drawY, 32, 32, 0, animation, angle);
            if (damaged)
            {
