...
namespace MiswGame2008
{
    public class Opera : Enemy
    {
        ...
        public Opera(Game game, double x, double y)
            : base(game, x, y)
        {
            angle = 90;
            vx = vy = 0;
            waitCount = game.Random.Next(150, 180);
            waitX = x;
            waitY = y;
            ...
            if (waitCount == 0)
            {
                ...
                else
                {
                    ...
                    else if (!Game.Player.IsDead)
                    {
                        if (Y < Game.Player.Y)
                        {
                            Bullet bullet1 = new EnemyBullet(Game, X + 32 * Utility.Cos(angle), Y - 32 * Utility.Sin(angle), 8, angle - 30);
                            Bullet bullet2 = new EnemyBullet(Game, X + 32 * Utility.Cos(angle), Y - 32 * Utility.Sin(angle), 8, angle - 15);
                            Bullet bullet3 = new EnemyBullet(Game, X + 32 * Utility.Cos(angle), Y - 32 * Utility.Sin(angle), 8, angle);
                            Bullet bullet4 = new EnemyBullet(Game, X + 32 * Utility.Cos(angle), Y - 32 * Utility.Sin(angle), 8, angle + 15);
                            Bullet bullet5 = new EnemyBullet(Game, X + 32 * Utility.Cos(angle), Y - 32 * Utility.Sin(angle), 8, angle + 30);
                            Game.AddEnemyBullet(bullet1);
                            Game.AddEnemyBullet(bullet2);
                            Game.AddEnemyBullet(bullet3);
                            Game.AddEnemyBullet(bullet4);
                            Game.AddEnemyBullet(bullet5);
        ...
        public override void Draw(IGraphics graphics)
        {
            ...
            graphics.DrawObject(Image.Browser, drawX, drawY, 32, 32, 2, 0, angle);
            if (damaged)
            {
