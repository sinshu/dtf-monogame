...
namespace MiswGame2008
{
    public class Ie : Enemy
    {
        ...
        public Ie(Game game, double x, double y)
            : base(game, x, y)
        {
            isDead = false;
            angle = 90;
            ...
            if (waitCount == 0)
            {
                ...
                else
                {
                    ...
                    else if (!Game.Player.IsDead)
                    {
                        if (Y < Game.Player.Y)
                        {
                            Bullet bullet = new EnemyBullet(Game, X + 32 * Utility.Cos(angle), Y - 32 * Utility.Sin(angle), 8, angle);
        ...
        public override bool Hit()
        {
            ...
            for (int i = 0; i < 8; i++)
            {
                ...
                Effect debris = new Debris(Game, x, y, 0, 255, 255, speed, angle);
                Game.AddEffect(debris);
            }
            Effect effect = new BigExplosionEffect(Game, X, Y, Game.Random.Next(360), 192, 255, 255);
        ...
        public override void Draw(IGraphics graphics)
        {
            ...
            graphics.DrawObject(Image.Browser, drawX, drawY, 32, 32, 0, 0, angle);
        }
